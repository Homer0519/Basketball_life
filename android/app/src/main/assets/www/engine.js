...
【写作铁律】爽文风格，写画面不写总结。每次只推一个场景，最多1-2个事件。末尾必须输出A/B/C/D选项。遇到比赛、冲突、重要对话、训练选择等关键决策时立刻给出选项，不要拖延。用「你」称呼玩家。禁用「就在这时」「突然」「他知道」。叙事800-2000字。R18场景强制slow节奏、>=500字直白露骨描写。【格式】关键词语用**粗体**（双星号），情绪词用*斜体*（单星号），R18淫秽词必须**加粗**。`;
...
class GameEngine {
  ...
  _userPrompt(state,input){
    ...
    p.push('## 行动\n'+input);
    if(state.pace==='fast')p.push('\n[快节奏：300-500字，1个事件，立刻给选项]');
    else if(state.pace==='medium')p.push('\n[中节奏：500-800字，1-2个事件，遇到决策立刻给选项]');
    else if(state.pace==='slow')p.push('\n[慢节奏：800-2000字，2-3个事件，R18>=500字]');
